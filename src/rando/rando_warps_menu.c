...
static menu_t *create_entrance_submenu(rando_scene_t * scene, uint32_t data){
    ...
    for (int i = 0; i < scene->entrance_cnt; i++){
        rando_entrance_t *ent = &((scene->entrances)[i]);
        menu_button_add(ent_menu, 0, i+1, ent->name, activate_warp_proc, (void *)(data | ((ent->index) & 0xFF)));
    }
    return ent_menu;
}

static menu_t *create_scene_submenu(rando_scene_category_t * cat){
    ...
    for (int i = 0; i < cat->scene_cnt; i++){
        rando_scene_t *scene = &((cat->scenes)[i]);
...
static menu_t *create_warps_submenu(rando_scene_category_t * scene_categories, int size){
    ...
    for(int i = 0; i<size; i++){
        rando_scene_category_t *cat = &scene_categories[i];
...
menu_t *create_warps_menu(){
    return create_warps_submenu(scene_categories, sizeof(scene_categories)/sizeof(rando_scene_category_t));
}